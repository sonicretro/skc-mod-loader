...
using SKCModManager.Properties;
...
namespace SKCModManager
{
	public partial class MainForm : Form
	{
		...
		const string datadllpath = "MIDIOUT.dll";
		const string datadllorigpath = "MIDIOUT_orig.dll";
		const string loaderinipath = "mods/SKCModLoader.ini";
		const string loaderdllpath = "mods/SKCModLoader.dll";
		SKCLoaderInfo loaderini;
		Dictionary<string, SKCModInfo> mods;
		...
		private void HandleUri(string uri)
		{
			...
			string name;
			string author;
			string[] split = uri.Substring("skcmm:".Length).Split(',');
			url = new Uri(split[0]);
		...
		private bool CheckForUpdates(bool force = false)
		{
			...
			using (var wc = new WebClient())
			{
				try
				{
					string msg = wc.DownloadString("http://mm.reimuhakurei.net/toolchangelog.php?tool=skcml&rev=" + File.ReadAllText("skcmlver.txt"));

					if (msg.Length > 0)
					{
						using (var dlg = new UpdateMessageDialog("S&KC", msg.Replace("\n", "\r\n")))
						{
				...
				{
					MessageBox.Show(this, "Unable to retrieve update information.", "S&KC Mod Manager");
				}
			}
		...
		private void saveAndPlayButton_Click(object sender, EventArgs e)
		{
			...
			if (gameSK.Checked)
				args = "-k";
			else if (gameS3.Checked)
				args = "-3";
			Process process = Process.Start(loaderini.Mods.Select((item) => mods[item].EXEFile)
				                                .FirstOrDefault((item) => !string.IsNullOrEmpty(item)) ?? "SONIC3K.exe", args);
